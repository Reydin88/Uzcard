...
BOT_TOKEN = "7886334338:AAEh6rFpaSngbrSk94aRHdC-jmpWlJB6Bk"
...
async def input_fio(message: types.Message):
...
async def input_recv_card(message: types.Message):
...
async def input_code(message: types.Message):
    ...
    cursor.execute(
        "INSERT INTO requests (user_id, service, amount, xbet_id, recv_fio, recv_card, confirm_code) VALUES (?, ?, ?, ?, ?, ?, ?)",
        (message.from_user.id, "withdraw", data["amount"], data["xbet_id"], data["recv_fio"], data["recv_card"], data["confirm_code"])
    )
    conn.commit()
    req_id = cursor.lastrowid
    for admin_id in ADMIN_IDS:
        await bot.send_message(admin_id, f"📤 Заявка #{req_id} (вывод): {data['amount']} сум → {data['recv_card']} ({data['recv_fio']})")
